...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        public float MeshBoundsScale = 1.4f;
        ...
        public void InitializeMeshes(CharacterSettings newSettings = null, bool resetAll = true)
        {
            ...
            foreach(var obj in childTransforms)
            {
                ...
                if (objType != "ACCESSORY" && objType != "HAT" && objType != "PANTS" && objType != "SHIRT" && objType != "SHOES" && objType != "ITEM1" && !string.Equals(objType, "hips", StringComparison.OrdinalIgnoreCase))
                {
                    if (skinnedMesh == null)
                        continue;
                ...
                }else if(string.Equals(objType,"hips", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    headHip = allHips.First(f => string.Equals(f.name, "head", StringComparison.OrdinalIgnoreCase));
                ...
                else if(objType == "HAT" || objType == "SHIRT" || objType == "PANTS" || objType == "SHOES" || objType == "ACCESSORY" || objType == "ITEM1")
                {
                    ...
                    skinnedMesh.gameObject.SetActive(false);
                    var clothesAnchor = clothesAnchors.Find(f => string.Equals(f.partType.ToString(), objType, StringComparison.OrdinalIgnoreCase));
        ...
        public void UpdateSkinnedMeshesOffscreenBounds()
        {
            ...
            foreach (SkinnedMeshRenderer mesh in allMeshes)
            {
                ...
                if (!UpdateWhenOffscreenMeshes && MeshBoundsScale >= 1.0f)
                {
                    ...
                    IEnumerator UpdateBounds()
                    {
                       ...
                       //     yield return new WaitForSeconds(0.1f);
                        ...
                        Vector3 extents = mesh.localBounds.extents * MeshBoundsScale;
        ...
        public void HideParts(string[] parts)
        {
            foreach (string p in parts)
            {
                ...
                {
                    if (string.Equals(cp.name, p, StringComparison.OrdinalIgnoreCase))
                    {
        ...
        public void EditorSavePreBuiltPrefab(bool bakeBlendshapes = false)
        {
            ...
            if (path != string.Empty)
            {
                ...
                prebuiltPrefab.name = string.Format("{0} - PreBuilt #{1}", this.gameObject.name, DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString());
        ...
        public void BakeCharacter(bool usePreBuiltMeshes = false, bool bakeBlendshapes = false)
        {
            ...
            {
                if (string.Equals(cm.name, "combined", StringComparison.OrdinalIgnoreCase))
